...
function AnimatedTimelineLine() {
  ...
  return (
    <div ref={ref} className="absolute left-4.75 top-2 bottom-2 hidden sm:block">
      <div className="w-px h-full bg-border/30" />
      <motion.div
        className="absolute top-0 left-0 w-px origin-top"
        style={{ scaleY, height: '100%', background: 'linear-gradient(to bottom, #0891b2, #6366f1, transparent)' }}
      />
    </div>
  )
}
        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {t.experience.jobs.map((job, i) => (
              ...
              >
                <TimelineDot isMilitary={'isMilitary' in job && job.isMilitary} index={i} />
