...
const inputError = 'border-red-400/60 focus:border-red-400/60 focus:ring-red-400/10'
...
export default function ContactForm() {
  ...
  const schema = Yup.object({
    ...
    email:   Yup.string().email(f.errors.emailInvalid).required(f.errors.emailRequired),
    subject: Yup.string().required(f.errors.subjectRequired),
    message: Yup.string().min(20, f.errors.messageTooShort).required(f.errors.messageRequired),
  })
  ...
  return (
    <form onSubmit={formik.handleSubmit} noValidate className="space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-text-muted mb-1.5">{f.name.label}</label>
          <input
        ...
        <div>
          <label className="block text-xs font-medium text-text-muted mb-1.5">{f.email.label}</label>
          <input
            type="email"
      ...
      {/* Subject */}
      <div>
        <label className="block text-xs font-medium text-text-muted mb-1.5">{f.subject.label}</label>
        <input
      ...
      {/* Message */}
      <div>
        <label className="block text-xs font-medium text-text-muted mb-1.5">{f.message.label}</label>
        <textarea
          rows={5}
